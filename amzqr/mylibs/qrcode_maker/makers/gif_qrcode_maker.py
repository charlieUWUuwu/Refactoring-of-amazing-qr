...
class GIFQRCodeMaker(BaseQRCodeMaker):
    ...
    def run(self):
        ...
        try:
            ...
            import imageio
            im = Image.open(self._params.picture)
            ...
            ims = [Image.open(pic) if self._params.colorized else Image.open(pic).convert('L') for pic in imsname]

            qr_name = os.path.join(self._params.save_dir, os.path.splitext(os.path.basename(self._params.picture))[0] + '_qrcode.gif') if not self._params.save_name else os.path.join(self._params.save_dir, self._params.save_name)
